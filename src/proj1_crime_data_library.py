...
def remove_outliers_iqr(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    where outliers are defined as values below Q1 - 1.5*IQR or above Q3 + 1.5*IQR.
...
def generate_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Generate new derived features (ex: create a ratio between two numeric columns if they exist).
...
def generate_timestamped_filename(base_name: str, extension: str = ".csv") -> str:
    ...
    where the timestamp follows the format: YYYY-MM-DD_HH-MM-SS.
