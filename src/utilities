...
class StorageManager:
    ...
    def show_info(self):
        print(f"Managing files inside: {self._path}")

    def __str__(self):
        return f"StorageManager(path='{self._path}')"
...
class SystemController:
    """Coordinates all storage and utility operations."""

    def __init__(self, base_path):
        """Initialize the system with key utility components."""
        from storage import StorageManager
        from utilities import Compressor, Encryptor, BackupManager, Logger
    ...
    def full_backup(self):
        """Perform a full backup: compress, encrypt, and store files."""
        self._logger.log("Starting full backup process...")

        files = self._storage.list_files()
        compressed = self._compressor.compress_all(files)
        encrypted = self._encryptor.encrypt_all(compressed)
        self._backup.store(encrypted)

        self._logger.log("Backup completed successfully.")
        return "✅ Backup completed."

    def system_status(self):
        """Return system information."""
        return {
            "storage_path": self._storage.path,
            "files": len(self._storage.list_files()),
            "last_backup": self._backup.last_backup_time
        }

    def __str__(self):
        return f"SystemController(base_path='{self._storage.path}')"
